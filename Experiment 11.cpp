...
node* delnode(node *root,int x)
{
    if(root==NULL)
        cout<<"Node not found";
    else if(root->info==x && root->right==NULL && root->left==NULL)
    {
        delete root;
        root=NULL;
        return root;
...
node* height(node *root,int x)
{
    ...
    else if(root->info==x)
    {
        ...
        root->left=height(root->left,x);
    else
        root->right=height(root->right,x);
}
int main()
{
    node *root = NULL;
